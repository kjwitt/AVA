...
class Calendar:
   ...
   def update(self):
      ...
      self.time_min_bin = num2binarray(datetime.datetime.now().time().minute,6)
...
class UI(Frame):
   def __init__(self, parent):
      ...
      self.configure(background = slurple)
      self.init_clock()
   def init_clock(self):
      lbl_date = Label(self, text=self.ava.cal.date_str, fg=kraagle, bg=slurple,font='Consolas')
      lbl_date.grid(sticky=W, pady=4, padx=5,column=0,row=5)
